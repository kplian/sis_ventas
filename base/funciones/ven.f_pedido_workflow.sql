CREATE OR REPLACE FUNCTION ven.f_pedido_workflow (
...
BEGIN
    ...
    select
    ped.fecha, ped.codigo, ped.id_proceso_wf, ped.id_estado_wf, ped.estado
    into
    v_rec_ped
    ...
    IF  (p_parametros->'operacion')::varchar = 'verificar' THEN
        ...
        -------------------------
        --Verificar que se haya registrado productos
        if not exists(select 1 from ven.tpedido_det
        			where id_pedido = (p_parametros->'id_pedido')::integer) then
        	raise exception 'No se ha registrado ningún Producto al Pedido.';
        end if;
     ...
     ELSIF (p_parametros->'operacion')::varchar = 'siguiente' THEN  
        ...
        where id_pedido = (p_parametros->'id_pedido')::integer;
    ...
    ELSIF (p_parametros->'operacion')::varchar = 'anterior' THEN
        ...
        v_respuesta = pxp.f_agrega_clave(v_respuesta,'mensaje','Se retrocedió el Pedido al estado anterior)'); 
